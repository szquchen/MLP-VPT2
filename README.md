...
(c) Then simply execute `./run`

### (2) Run DVPT2+GVPT2 and (optionally) compute IR/Raman intensities in folder step2:

Copy the `XXX.out` files from `step1` to `step2`. Modify the script `run` to adjust the input arguments (explained at the beginning of "main()" in `gvpt2.py`. Then simply execute `./run`
